...
export default class Game extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="game-container">
        ...
        <footer id="footer">
          <p>Click on tile to expose its content, right click or
            <span> alt+left click </span>to mark the mine</p>
  ...
  levelChangeHandler = (value) => {
    this.stopTimer();
    this.setState({timePassed: 0, status: 'playing', current: value});
  }
  ...
  reset = () => {
    this.stopTimer();
    this.setState({timePassed: 0, status: 'reset', flags: 0});
  }
